...
interface LabPurchaseValues {
    ...
    items: { purchaseOrderNumber: string; description: string; quantity: number; unitPrice: number; totalPrice: number }[];
  }
  ...
  const LabPurchaseOrder = () => {
    const form = useForm<LabPurchaseValues>({
      defaultValues: {
        ...
        labEmail: "",
        labContactPerson: "",
        items: [{ purchaseOrderNumber: "", description: "", quantity: 0, unitPrice: 0, totalPrice: 0 }], // Default item
      },
    });
  
    const [items, setItems] = useState<{ purchaseOrderNumber: string; description: string; quantity: number; unitPrice: number; totalPrice: number }[]>([
      { purchaseOrderNumber: "", description: "", quantity: 0, unitPrice: 0, totalPrice: 0 },
    ]);
  
    const handleAddRow = () => {
      setItems([...items, { purchaseOrderNumber: "", description: "", quantity: 0, unitPrice: 0, totalPrice: 0 }]);
    };
  ...
  const handleSubmit = async (values: LabPurchaseValues) => {
    try {
      const data = { ...values, items }; // Include items in submitted data
      console.log("Submitted Values:", data);
      toast.success("Submission successful!");
      form.reset();
      setItems([{ purchaseOrderNumber: "", description: "", quantity: 0, unitPrice: 0, totalPrice: 0 }]); // Reset items
    } catch (error) {
  ...
  return (
    <div className="flex w-screen h-screen bg-gray-100">
      ...
      <div className="flex-1 flex justify-center items-center p-8">
        <Card className="p-8 w-full max-w-[960px] max-h-[700px] shadow-lg mt-20">
          <div className="flex flex-col items-center mb-4">
            ...
            <h1 className="text-xl font-bold py-1">Laboratory Purchase Order Form</h1>
          ...
          <div className="overflow-y-auto max-h-[430px] mb-1">
            <Form {...form}>
              <form onSubmit={form.handleSubmit(handleSubmit)} className="mb-4">
                      ...
                      <FormItem>
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent className="w-56">
                            {["Submitted", "Procurement Office", "Accounting Office", "Delivered", "Cancelled"].map((option) => (
                              <DropdownMenuCheckboxItem
                                key={option}
                ...
                <div>
                    <h4 className="text-s font-bold py-1">Supplier Information</h4>  
                </div>
                <div>
                  ...
                  <table className="min-w-full border border-gray-300">
                    <thead>
                      <tr className="bg-gray-200">
                        <th className="text-sm px-2 py-1 border">Purchase Order No.</th>
                        <th className="text-sm px-2 py-1 border">Description</th>
                        <th className="text-sm px-2 py-1 border">Quantity</th>
                        <th className="text-sm px-2 py-1 border">Unit Price (Php)</th>
                        <th className="text-sm px-2 py-1 border">Total Price (Php)</th>
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr key={index}>
                          <td className="border px-4 py-2">
                            <Input
                              ...
                              onChange={(e) => handleChange(index, "purchaseOrderNumber", e.target.value)}
                      ...
                      <tr>
                        <td colSpan={5}>
                         <div className="flex justify-center">
                            <Button onClick={handleAddRow} className="bg-gray-300 text-black hover:bg-sky-500 hover:text-white w-full">
                            Add New Row
                            </Button>
                        </div>
                        </td>
                      ...
                      <tr>
                        <td colSpan={3} className="border px-4 py-2"></td>
                        <td className="border px-4 py-2 text-sm">Shipping Cost: </td>
                        <td className="border px-4 py-2"></td>
                      </tr>
                      <tr>
                        <td colSpan={3} className="border px-4 py-2"></td>
                        <td className="border px-4 py-2 text-sm">Grand Total: </td>
                        <td className="border px-4 py-2"></td>
                      </tr>
                    </tbody>
                  </table>
