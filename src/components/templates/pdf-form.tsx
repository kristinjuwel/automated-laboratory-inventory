...
const PdfGenerator: React.FC<PdfGeneratorProps> = ({
  ...
  const handlePrint = () => {
    ...
    const hasImageAttachment = tableData.some(row => row[7]?.startsWith("data:image/"));
        ...
        didDrawCell: (data) => {
          if (data.row.index !== undefined && data.column.index === 0) {
            ...
            if (imageBase64 && imageBase64.startsWith("data:image/")) {
              ...
              const remainingHeight = pageHeight - (data.cell.y + data.cell.height + margin);
              ...
              if (remainingHeight > margin) {
                ...
                // Add the image
                try {
                  doc.addImage(imageBase64, "PNG", xPos, yPos, imgWidth, imgHeight);
                } catch (error) {
